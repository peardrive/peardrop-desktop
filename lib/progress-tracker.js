...
class ProgressTracker extends EventEmitter {
  constructor() {
    ...
    // Listeners attached per drive, so stopTracking can DETACH them.
    // Map<driveId, Array<{ core, event, handler }>>
    // Without this, stopTracking deleted the drive's state but left the
    // 'upload' listener bound to the core — hypercore then delivered an
    // in-flight upload after the teardown and the handler ran against
    // state that no longer existed, crashing the main process.
    this.handlersByDrive = new Map()
  }
  ...
  trackUploads(driveId, drive, totalBytes) {
    ...
    const uploadHandler = (index, byteLength, peer) => {
      ...
      // The drive may have been stopped between hypercore queueing this
      // event and delivering it. No entry means we are no longer tracking,
      // so there is nothing to record — and an uncaught throw here takes
      // down the whole main process with an OS-level error dialog.
    ...
    // Also keep it against the driveId, which is all stopTracking receives.
    const registered = this.handlersByDrive.get(driveId) || []
    registered.push({ core: blobs, event: 'upload', handler: uploadHandler })
    this.handlersByDrive.set(driveId, registered)
  ...
  stopTracking(driveId) {
    // Detach listeners FIRST, so no further events can arrive while the
    // state below is being dismantled.
    const registered = this.handlersByDrive.get(driveId)
    if (registered) {
      for (const { core, event, handler } of registered) {
        try {
          core.removeListener(event, handler)
        } catch (err) {
          console.warn('[ProgressTracker] Could not detach listener', err.message)
        }
      }
      this.handlersByDrive.delete(driveId)
    }
