...
(function () {
    ...
    // Set by pickFile() for the "import an image" path, which runs WITHOUT
    // opening the camera modal. Kept separate from onResultCallback so the
    // two entry points can never overwrite each other's result handler.
    let fileOnlyCallback = null;
    ...
    function init() {
        ...
        fileInput.addEventListener('change', async () => {
            ...
            try {
                ...
                if (fileOnlyCallback) {
                    // Import-image path: no modal is open, so deliver()'s
                    // close() would be wrong. Hand the result straight back.
                    const cb = fileOnlyCallback;
                    fileOnlyCallback = null;
                    cb(text);
                } else {
                    deliver(text);
                }
    ...
    /**
     * Decode a QR from an image file WITHOUT opening the camera scanner.
     *
     * Exists because the Receive modal used to click #qrFileInput directly.
     * That input belongs to this module, and its change listener is bound in
     * init() — which only runs from open(). So on a cold app, importing an
     * image did nothing at all, and even once bound the decoded text had no
     * callback to reach. Callers now go through this instead of reaching
     * into another module's DOM.
     *
     * @param {{ onResult?: (text: string) => void }} opts
     */
    function pickFile({ onResult } = {}) {
        init();                     // binds the change listener if it hasn't been
        fileOnlyCallback = typeof onResult === 'function' ? onResult : null;
        fileInput.value = '';       // so re-picking the same file still fires
        fileInput.click();
    }

    window.openQrScanner = open;
    window.pickQrFile = pickFile;
})();
