 ...
 * 'share-progress' - { driveId, filesDone, filesTotal, bytesDone,
 *                      bytesTotal, currentFile } during createDrive
...
class HyperdriveManager extends EventEmitter {
  constructor(options = {}) {
    ...
    // Share builds the user asked to cancel. createDrive checks this
    // between files and mid-stream; see cancelShare().
    this.cancelledShares = new Set()
   ...
   */
  /**
   * Cancel an in-flight createDrive. The build aborts at the next chunk
   * boundary, the partial drive is deleted, and createDrive rejects with
   * err.cancelled === true.
   * @param {string} driveId - from the 'share-progress' event
   */
  cancelShare(driveId) {
    if (!driveId) return false
    this.cancelledShares.add(driveId)
    console.log('[HyperdriveManager] Share cancel requested', { driveId })
    return true
  }
  ...
  async createDrive(files, options = {}) {
    ...
    try {
      ...
      for (const file of files) {
        // CANCEL CHECK (between files) — cheap, and the only check that
        // matters for a many-small-files share.
        if (this.cancelledShares.has(driveId)) {
        ...
        // Bytes written for THIS file so far — the running total plus this
        // is the overall figure reported while a file is still in flight.
        let fileBytesDone = 0
        ...
        await new Promise((resolve, reject) => {
          // SACRED-ADJACENT: progress reporting only. A listener on 'data'
          // does not consume the stream or alter the pipe — 'data' fires
          // regardless once piped. Without this, a single large file emits
          // ONE progress event, at completion: the bar sits at 0% for a
          // 3GB video and then jumps to 100%.
          let sentAt = 0
          readStream.on('data', (chunk) => {
            // CANCEL CHECK (mid-file) — without this a single large file
            // could not be interrupted until it finished.
            if (this.cancelledShares.has(driveId)) {
              const err = new Error('Share cancelled')
              err.cancelled = true
              readStream.destroy()
            ...
            fileBytesDone += chunk.length
            const now = Date.now()
            if (now - sentAt < 100) return    // cap IPC chatter at ~10/sec
            sentAt = now
            this.emit('share-progress', {
              driveId,
              filesDone: fileEntries.length,
              filesTotal: files.length,
              bytesDone: totalBytes + fileBytesDone,
              bytesTotal: options.totalBytes || 0,
        ...
        // SACRED-ADJACENT: progress reporting only. This emit does not
        // touch the stat/pipe/ordering logic above it in any way — remove
        // this block and createDrive behaves exactly as it did before.
        // Bytes, not file count: one big file among many small ones would
        // park a count-based bar at 99% for most of the wait.
        this.emit('share-progress', {
          driveId,
          filesDone: fileEntries.length,
          filesTotal: files.length,
      ...
      const peardropManifest = {
        ...
        // Placeholder name for a multi-file share. Was `${n} files`, which
        // duplicated the count already shown in the card's detail row and
        // left every multi-file share looking identical in the list.
        name: options.name || (files.length === 1 ? fileEntries[0].name : 'Folder'),
        created: Date.now(),
      ...
      swarm.on('connection', (socket, peerInfo) => {
        ...
        // SACRED-ADJACENT: adds a field to an existing event. Nothing about
        // the swarm join or store.replicate() path changes. The full key is
        // needed by the File Info panel; peerId stays truncated for logs.
        this.emit('peer-connected', { driveId, peerId, publicKey: peerInfo?.publicKey?.toString('hex') || null, at: Date.now() })
        
        store.replicate(socket)
        
        socket.on('close', () => {
          console.log('[HyperdriveManager] Peer disconnected', { driveId, peerId })
          this.emit('peer-disconnected', { driveId, peerId, publicKey: peerInfo?.publicKey?.toString('hex') || null, at: Date.now() })
        })
      ...
      // The announce is the moment the share becomes FINDABLE. Until it
      // lands, the drive is open locally but no peer holding the link can
      // reach it — so the UI must not call it "Active" yet. This used to be
      // a console.log only, which is why a card went green up to 11s before
      // it was actually reachable.
      topic.flushed().then(() => {
        ...
        this.emit('drive-announced', { driveId, at: Date.now() })
      }, (err) => {
        console.warn('[HyperdriveManager] DHT announce failed', { driveId, error: err?.message })
        this.emit('drive-announce-failed', { driveId, error: err?.message || 'announce failed' })
      })
      ...
      this.cancelledShares.delete(driveId)
      ...
      if (error && error.cancelled) {
        // User-initiated: leave NO trace. An ERRORED entry would show up
        // in the Shares list as a failed share the user never made.
        console.log('[HyperdriveManager] Share cancelled, removing partial drive', { driveId })
        ...
        try {
          ...
          console.warn('[HyperdriveManager] Could not remove cancelled drive folder', rmErr.message)
        }
        throw error
      }
  ...
  async openDrive(shareLink) {
    ...
    // Hyperswarm routinely holds several connections to the same share, so a
    // single socket closing does NOT mean the sender is gone. Emitting on
    // every close made the UI park a download that still had live peers, and
    // a stale socket tearing down just after a Resume re-parked it instantly.
    // Only the LAST peer leaving is a disconnect.
    let livePeers = 0
    swarm.on('connection', (socket) => {
      peerConnected = true
      livePeers++
      console.log('[HyperdriveManager] Connected to peer for download', { driveId, livePeers })
      store.replicate(socket)
      ...
      // 'close' can in principle fire more than once per socket; counting it
      // twice would drive livePeers negative and suppress a real disconnect.
      let counted = false
      socket.on('close', () => {
        if (counted) return
        counted = true
        livePeers = Math.max(0, livePeers - 1)
        if (livePeers > 0) {
          console.log('[HyperdriveManager] A download peer left, others remain', { driveId, livePeers })
          return
        }
        peerDisconnected = true
        console.log('[HyperdriveManager] Download peer disconnected (last peer)', { driveId })
        this.emit('download-peer-disconnected', { driveId })
      })
    })
    
    // `findingPeers()` is what makes `drive.update({ wait: true })` actually
    // wait. It tells the core that peers may still arrive, so "I have no
    // peers, therefore I am up to date" stops being a valid conclusion until
    // discovery settles and `done()` is called.
    //
    // This used to read `// REMOVED: done() - Keep seeking peers indefinitely`,
    // which did the opposite of its stated intent: with no findingPeers() at
    // all, update() resolved against an empty metadata core within
    // milliseconds of a socket opening, and the drive read as an EMPTY SHARE.
    // That is how a 0-file "Untitled Share" ghost entry got written to the
    // manifest and announced as "now seeding". createDrive has always used
    // this pattern (see the share path above) — only the receive paths lacked it.
    //
    // `done` must not be called before update() starts, hence flush is no
    // longer awaited here; nothing between this point and the update depends
    // on it. Behaviour when no peer is ever found is unchanged: discovery
    // settles, done() fires, update() resolves, peerConnected stays false and
    // the drive is left in seeking state.
    const done = drive.findingPeers()
    const topic = swarm.join(drive.discoveryKey)
    ...
    console.log('[HyperdriveManager] Swarm join issued, waiting for drive update...', { driveId, peerConnected })
    ...
    //
    // A connected socket is NOT proof the drive synced. Every share this app
    // creates carries a `.peardrop.json`, so "peer connected, but no manifest
    // AND no files" can only mean the metadata core has not replicated yet.
    // Persisting that is what wrote the 0-file "Untitled Share" ghost. Stay in
    // seeking state instead — no entry means nothing to clean up later.
    const driveHasData = !!manifest || files.length > 0
    if (peerConnected && !driveHasData) {
      console.warn('[HyperdriveManager] Peer connected but drive has not synced — staying in seeking state, not persisting',
        { driveId, coreLength: drive.core?.length ?? null })
    }
    if (peerConnected && driveHasData) {
  ...
  async _resumeDrive(driveId, metadata) {
    ...
    // Same signal on the resume path — see the note in createDrive.
    topic.flushed().then(() => {
      ...
      console.warn('[HyperdriveManager] DHT announce failed on resume', { driveId, error: err?.message })
      this.emit('drive-announce-failed', { driveId, error: err?.message || 'announce failed' })
    })
  ...
  async _resumeSeekingDrive(driveId, metadata) {
    ...
    let peerConnected = false
    // Same multi-connection rule as the fresh-open path: only the last peer
    ...
    // Same findingPeers() requirement as the fresh-open path above — without
    // it update({ wait: true }) resolves against an empty core and the resumed
    // download reads its own share as empty.
    const done = drive.findingPeers()
    const topic = swarm.join(drive.discoveryKey)
    ...
    try {
      ...
      if (files.length > 0) {
        shareName = files.length === 1 ? files[0].name : 'Folder'
      }
    }
    ...
    // Same rule as the fresh-open path: a socket is not a synced drive. If the
    // manifest and the listing both came back empty, replication has not
    // delivered anything yet — emitting drive-ready-to-download here would
    // start a download of nothing and overwrite a real entry with an empty one.
    const resumeHasData = !!manifest || files.length > 0
    if (peerConnected && !resumeHasData) {
      console.warn('[HyperdriveManager] Resumed peer connected but drive has not synced — not emitting ready-to-download',
        { driveId, coreLength: drive.core?.length ?? null })
    }
    if (peerConnected && resumeHasData) {
      await this.addDriveEntry({
