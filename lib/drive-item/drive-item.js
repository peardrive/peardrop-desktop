...
(function(root, factory) {
  ...
  class DriveItem {
    ...
    _patchProgressInPlace() {
      ...
      if (speedEl) speedEl.textContent = data.speed > 0 ? formatSpeed(data.speed) : '';
    ...
    _buildContentHTML(data, show) {
      ...
      if (show.includes('title')) {
        ...
        // would be noise.
        const _isMoving = data.status === 'downloading' || data.status === 'sharing';
        if (_isMoving && data.speed > 0) {
          _lineParts.push(`<span class="drive-item-status-dash" aria-hidden="true">&ndash;</span>`);
          _lineParts.push(`<span class="drive-item-status-speed">${formatSpeed(data.speed)}</span>`);
        }
      ...
      if (_isInProgress) {
        ...
        parts.push(`<button type="button" class="drive-item-row-action drive-item-row-action-cancel" data-row-action="remove" aria-label="Cancel"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.6" stroke-linecap="round"><path d="M7 7l10 10M17 7L7 17"/></svg></button>`);
      } else if (_isFailed) {
