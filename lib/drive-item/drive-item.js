...
(function(root, factory) {
  ...
  class DriveItem {
    ...
    _deriveStatus(data, explicitStatus) {
      ...
      //
      // 'sharing' is in the list for a DIFFERENT reason, and it is the whole
      // point of this guard. The host only ever sets it when it KNOWS the
      // direction: an 'upload-progress' event carries the remote peer's id
      // for an upload and 'self' for a download, so the renderer has already
      // decided. Without the guard, the branch below saw `speed > 0` on a
      // card whose type is 'download' (anything you received and are now
      // re-seeding) and relabelled an UPLOAD as "Downloading" — no progress
      // bar, someone else's transfer, the desktop looking like it did not
      // have its own file.
      //
      // Direction is a fact the event already carried. It must not be
      // re-guessed from a speed number that says nothing about which way the
      // bytes are going.
      ...
      // Sharing: an outgoing drive with peers connected.
      // Accepts 'share' as well as 'upload': normalizeDrive in renderer.js
      // produces 'share', so testing 'upload' alone made this branch
      // unreachable from the app — two vocabularies for one idea.
      else if ((data.type === 'upload' || data.type === 'share') && data.peers > 0) {
        derived = 'sharing';
      }
    ...
    _injectStyles() {
      ...
      style.textContent = `
        ...
        .drive-item-menu {
          /* display:none when closed, NOT just visibility:hidden.
             visibility:hidden still LAYS THE ELEMENT OUT, and an
             absolutely-positioned box at top:100% contributes to the
             scroll container's overflow — so every closed menu silently
             extended .scroll-list's scrollHeight, hanging below its row.
             Opening a menu moves it to document.body, the list's height
             shrank, and a list scrolled to the bottom had its scrollTop
             clamped: the list jumped up "to show the menu" and snapped back
             on dismiss. display:none removes it from layout entirely, so
             the height never changes and there is nothing to clamp.
             Both open paths add .open BEFORE measuring offsetHeight, so
             positioning still measures a laid-out element. */
          display: none;
          position: absolute;
          top: 100%;
          right: 0;
    ...
    _patchProgressInPlace() {
      ...
      // Label + colour must track the same rules as the builder, or a card
      // patched in place keeps the wording from its last full render.
      const label = el.querySelector('.drive-item-status-label');
      const lineEl = el.querySelector('.drive-item-status-line');
      if (label) {
        if (data.status === 'downloading' && percent != null) {
          label.textContent = `Downloading (${percent}%)`;
        } else if (data.status === 'sharing' && percent != null && percent < 100) {
          label.textContent = `Sharing (${percent}%)`;
          if (lineEl) lineEl.dataset.status = 'sharing-progress';
        } else if (data.status === 'sharing') {
          // Flips live as the rate rises and falls. data-status carries the
          // colour, and the patcher owns it here because a status that never
          // changes ('sharing' throughout) will not trigger a re-render.
          const serving = data.speed > 0;
          label.textContent = serving ? 'Sharing' : 'Active';
          if (lineEl) lineEl.dataset.status = serving ? 'sharing-progress' : 'sharing';
        }
      }

      const speedEl = el.querySelector('.drive-item-status-speed');
      const dashEl = el.querySelector('.drive-item-status-dash');
      // MUST match _isMoving in the builder above, or the patcher hides a
      // rate the renderer just emitted (or vice versa). A seeding share is
      // included: it shows its rate while bytes flow and falls back to a
      // plain "Active" when speed returns to 0.
    ...
    _buildContentHTML(data, show) {
      ...
      if (show.includes('title')) {
        ...
        } else if (data.status === 'sharing' && data.speed > 0) {
          // Bytes are leaving right now: say SHARING, not "Active".
          // "Active" means "available if someone wants it"; this is the
          // stronger claim that a transfer is happening, and it is the
          // difference the user actually wants to see at a glance.
          // Amber via 'sharing-progress', matching "Sharing (98%)" — the
          // same in-progress colour the design already uses.
          _statusText = 'Sharing';
          _dataStatus = 'sharing-progress';
        } else if (data.status === 'sharing') {
          _statusText = 'Active';
        ...
        // Emitted for any state that CAN move, including a plain 'sharing'
        // card — a finished download that is now seeding sits at progress 1,
        // so 'sharing-progress' (percent < 100) never applies to it and it
        // could never show an upload rate at all. That is why serving a file
        // to a phone showed "Active" and nothing else.
        //
        // The spans are created even at speed 0 and hidden until there is a
        // reading: _patchProgressInPlace updates numbers WITHOUT rebuilding
        // the DOM, so anything not emitted here can never be filled in later.
        //
        // An earlier comment warned that keying off 'sharing' left a stale
        // rate next to "Active" forever. That was true while nothing ever
        // reset the speed — 'upload-complete' was emitted but unsubscribed.
        // The renderer now zeroes it on completion and when the last peer
        // leaves, so `speed > 0` is trustworthy.
