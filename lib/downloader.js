...
async function downloadFromDrive(drive, options = {}) {
    ...
    //
    // CANCELLABLE. drive.list() waits on metadata blocks from the peer, so
    // with the sender offline this blocks indefinitely — and the per-file
    // aborter further down does not exist yet. A cancel arriving here used to
    // find nothing to abort and the download stayed wedged forever.
    const filesToDownload = [];
    const listStream = drive.list('/');
    registerAborter(() => {
        try { listStream.destroy?.(); } catch (_) { /* already gone */ }
    });

    // STALL WATCHDOG for the listing phase.
    //
    // The per-file watchdog further down only arms once a read stream exists,
    // so it never covered this loop. A drive that was resumed and announced,
    // but whose sender is actually gone, leaves this `for await` waiting on
    // metadata blocks that never arrive: no progress, no error, no rejection.
    // The download IPC never settles and the card sits on "Downloading"
    // forever — which is exactly what a resume-after-restart looked like.
    //
    // The timer resets on every entry, so a slow-but-live peer is never cut
    // off; only true silence trips it.
    let listStalled = false;
    let listTimer = null;
    const clearListStall = () => { if (listTimer) { clearTimeout(listTimer); listTimer = null; } };
    const armListStall = () => {
        clearListStall();
        listTimer = setTimeout(() => {
            listStalled = true;
            // Destroying ends the for-await rather than throwing from it
            // (same behaviour the cancel path relies on), so the flag above
            // is what tells us why we stopped.
            try { listStream.destroy?.(); } catch (_) { /* already gone */ }
        }, STALL_TIMEOUT_MS);
    };
    armListStall();
    ...
    // Silence, not cancellation: the peer never sent the file list. Fail
    // loudly so the caller can surface it, instead of proceeding with an
    // empty list and reporting a successful download of nothing.
    if (listStalled) {
        throw new FileStallError(
            `stalled: no file list for ${STALL_TIMEOUT_MS / 1000}s (sender may be offline)`,
            { phase: 'list', timeoutMs: STALL_TIMEOUT_MS },
        );
    }

    // Destroying the stream above makes the for-await finish rather than
    // throw, so re-check before carrying on with an empty list.
    if (isCancelled()) {
        const err = new Error('Download cancelled');
        err.cancelled = true;
        err.partialFiles = [];
